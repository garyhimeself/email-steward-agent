...
If the server rejects a login, that does not prove the password is wrong. Check whether the account permits third-party client login, whether the personal mailbox address is correct, whether an administrator policy blocks IMAP, and whether the one-time password was copied completely. A timeout, TLS error, or network error is a connection issue, not a password diagnosis. Do not paste a password, raw server log, or screenshot containing either into chat.

## Read-only IMAP check fails

Check your network and email address, then run the installer again. After a failed check, the installer removes its incomplete workspace and saved mailbox password so retrying starts cleanly. The expected Alibaba servers are IMAP SSL `imap.qiye.aliyun.com:993` and SMTP SSL `smtp.qiye.aliyun.com:465`. A connection failure does not mean email was changed. If the message says the read-only check failed after login, ask the mail administrator about mailbox availability rather than changing the password blindly. Do not switch to browser automation as a workaround.
