...
1. Download the public GitHub release or extract the ZIP to any temporary location.
2. Give the package to Codex and say: `Help me install this Agent.`
3. On Windows, let Codex start the installer after you confirm the chat summary; it opens a **separate secure PowerShell window** for the local password prompt. Do not open normal setup by double-clicking `installer/install_agent.bat`; that file is only for the credential-free `--preflight` check or Codex's secure-window launch. On macOS, open `installer/install_agent.command`. If macOS blocks it, right-click the file, choose **Open**, and approve it once. Before any password prompt, it displays a local network preflight: computer name, Alibaba IMAP address, DNS results, TLS reachability, and the current public IP when available. It does not log in, read email, ask for an email address, or save this diagnostic. The public IP is a current observation, not a fixed-IP promise.
...
5. In the Codex chat, choose a private workspace you can keep, review the displayed full path, and explicitly confirm it before setup begins. No workspace is created before this confirmation.
6. In the Codex chat, give your name, individual company email, preferred language, reply language, tone, and daily-brief choice once. Codex passes only those non-secret values to the installer. On Windows, it then opens the separate secure PowerShell window, which asks only for the third-party client password in a hidden local prompt and stores it in the operating-system credential store. The integrated terminal is never a password fallback.
...
Collect the workspace, daily-brief choice, and five non-secret profile values in the Codex chat. On Windows, start only the dedicated window path with `--secure-window`, `--workspace`, `--daily-brief`, `--name`, `--email`, `--preferred-language`, `--reply-language`, and `--reply-tone`; never start a normal interactive setup in Codex's integrated terminal. Never pass a third-party client password in a command, environment variable, file, or chat. The password is entered only through the separate secure PowerShell window.
